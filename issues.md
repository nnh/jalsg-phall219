...
1. ~~SAP未固定~~：2026-07-20、`docs/sap-edits-todo.md`の修正案をもとにSAP本体（Google Doc、第1〜5章）を編集・**固定完了**。OPENコメント4件は全て解決操作済み。fix-list必須3件・PRT齟齬2件・データ制約1件は全てSAP本体への反映または対応不要と確認済み（死因内訳の`xx(%)`表記は実データ未集計の想定通りのプレースホルダー、RFS対象集団#49を含める扱いはSAP本文の既存記載で説明として十分と最終判断）。SAP関連の対応は完了。詳細は[docs/sap-open-issues.md](docs/sap-open-issues.md)。
